...
registerBtn.onclick = async () => {
  ...
  if (!name || !username || !password || !passwordConfirm) {
    setStatus(registerStatus, "Navn, brukernavn og passord må fylles ut.");
    return;
  }

  if (password !== passwordConfirm) {
    setStatus(registerStatus, "Passordene er ikke like.");
    return;
  }

  if (password.length < 8) {
    setStatus(registerStatus, "Passordet må være minst 8 tegn.");
    return;
  }

  const internalEmail = `${username}@samnanger.local`;

  setStatus(registerStatus, "Oppretter bruker…");
  ...
  try {
    ...
    console.error(err);
    setStatus(registerStatus, "Kunne ikke opprette bruker. Prøv igjen.");
  }
};
...
loginBtn.onclick = async () => {
  ...
  if (!username || !password) {
    setStatus(loginStatus, "Brukernavn og passord må fylles ut.");
    return;
  }

  const email = `${username}@samnanger.local`;

  setStatus(loginStatus, "Logger inn…");
  ...
  try {
    ...
    if (!userSnap.exists()) {
      setStatus(loginStatus, "Brukerprofil mangler.");
      return;
    }

    const userData = userSnap.data();

    // 🔑 Trener slipper alltid inn
    ...
    if (userData.approved !== true) {
      ...
      loginBox.hidden = false;
      registerBox.hidden = true;
      setStatus(loginStatus, "Kontoen venter på godkjenning av trener.");
      return;
    }
    ...
    console.error(err);
    setStatus(loginStatus, "Feil brukernavn eller passord.");
  }
};

coachLoginBtn.onclick = async () => {
  const email = coachEmail.value.trim().toLowerCase();
  const password = coachPassword.value;

  if (!email || !password) {
    coachLoginStatus.textContent =
      "E-post og passord må fylles ut.";
    return;
  }

  coachLoginStatus.textContent = "Logger inn…";
  ...
  try {
    ...
    if (data.role !== "coach") {
      await signOut(auth);
      coachLoginStatus.textContent =
        "Denne brukeren er ikke trener.";
      return;
    }

    coachLoginStatus.textContent = "";
    console.log("✅ Logget inn som trener");
...
logoutBtn.addEventListener("click", async () => {
  console.log("🔴 LOGG UT KLIKKET");
  await signOut(auth);
  console.log("🔴 signOut ferdig");
});
...
if (!user) {
  console.log("Ingen bruker innlogget");
  showLoggedOutUI();
  return;
}
  ...
  if (!snap.exists()) {
    console.warn("Fant ikke bruker i Firestore for uid:", user.uid);
    setStatus(loginStatus, "Bruker finnes ikke i systemet.");
    return;
  }

  const data = snap.data();
  // 🔹 Last sesongmål for innlogget bruker
await loadSeasonGoal(user.uid);

  console.log("BRUKERDATA:", data);
  
    // 🔹 Vis hvem som er innlogget (øverst til høyre)
const userBox = document.getElementById("loggedInUser");
const userNameEl = document.getElementById("loggedInUserName");

if (userBox && userNameEl) {
  userNameEl.textContent = data.name || data.email || "Ukjent";
  userBox.hidden = false;
}

  // ✅ TRENER
  if (data.role === "coach") {
    console.log("✅ DU ER INNLOGGET SOM TRENER");
    setStatus(loginStatus, "Du er innlogget som trener.");
    authBox.hidden = true;
    appBox.hidden = false;
    return;
  }
